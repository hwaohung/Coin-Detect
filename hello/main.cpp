...
{	
	...
	for (int i = 0; i < files.size(); i++){
		base_path = "/home/johnny/Documents/Test/hello/Debug/sample/all/crop_filter/";
		files[i] = "台幣50元 反面.pcd";
		test(base_path+files[i]);
		return 0;
	}
	
	/*
	PointCloud<PFHSignature125>::Ptr PFH_features (new PointCloud<PFHSignature125>);
	compute_PFH_features(points, normals, 1, PFH_features, keypoints);
	record("Compute PFH features");
	*/
	
	/*
	PointCloud<FPFHSignature33>::Ptr FPFH_features (new PointCloud<FPFHSignature33>);
	compute_FPFH_features(points, normals, 1, FPFH_features, keypoints);
